...
def generate_text(prompt:str, max_new_tokens:int, temperature:float) -> str:
    model_inputs = tokenizer(prompt, return_tensors="pt").to("cuda") # cpu
...
prompt = '''In this article, we are going to explore cases where a driver has been the subject of a false arrest'''

generate_text(prompt, 500, 0.25)


end = time.time()
print(f"\nExecution time: {(end-start)/60:.02f} minutes\n")